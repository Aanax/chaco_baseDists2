...
def test(args, shared_model, env_conf, counter, num):
    ...
    log["{}_log".format(args["Training"]["env"])] = logging.getLogger(
        "{}_log".format(args["Training"]["env"])
    )
    #     d_args = vars(args)
    for k in args.keys():
    ...
    player = Agent(
        _model1, _model2, env, args, None, gpu_id
    )  # Agent(None, env, args, None)
    ...
    while True:
        ...
        if player.done and not player.info:
            ...
            log["{}_log".format(args["Training"]["env"])].info(
                "Time {0}, episode reward {1}, episode length {2}, reward mean {3:.4f}, episodes_total {4}".format(
            ...
            if args["Training"]["save_max"] and reward_sum >= max_score:
                ...
                if gpu_id >= 0:
                    with torch.cuda.device(gpu_id):
                        ...
                        torch.save(
                            state_to_save,
                            "{0}{1}{2}_{3}.dat".format(
                                args["Training"]["save_model_dir"],
                                args["Training"]["env"],
                                architecture_type,
                                reward_sum,
                            ),
