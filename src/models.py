...
class Level1(nn.Module):
    def __init__(self, args, shap, n_actions, device):
        ...
        # 32x40x40
        self.ConvLSTM_mu = ConvLSTMwithAbaseCell(
            input_dim=32,
            hidden_dim=32,
            kernel_size=(5, 5),
            bias=True,
        ...
        self.oracle.conv1.weight.data = (
            self.oracle.conv1.weight.data * args["Model"]["S_init_std_multiplier"]
        )
        for name, p in self.named_parameters():
            if "lstm" in name:
                if ("weight_ih" in name) or ("t_layer.weight" in name):
                    nn.init.xavier_uniform_(
                        p.data, gain=args["Model"]["lstm_init_gain"]
                    )
    ...
    def forward(self, x):
        ...
        #         print(s.shape)
        hx, cx = self.ConvLSTM_mu(
            s, (hx1, cx1), prev_action_logits
        ...
        actor_in = z.view(
            z.size(0), -1
        )  # T.cat((z.view(z.size(0), -1), a2_prev.view(a2_prev.size(0), -1)),1)
