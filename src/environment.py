...
def atari_env(env_id, env_conf, args):
    ...
    if env_id in mujoco_envs_list:
        ...
        if 'NoFrameskip' in env_id:
            ...
            env._max_episode_steps = args["Training"]["max_episode_length"] * args["Training"]["skip_rate"]
...
class MujocoDiscreteWrapper(gym.Wrapper):
    ...
    def __init__(self,env):
        ...
        self.action_space = Discrete(int(self.action_space.high[0]-self.action_space.low[0])) 
...
class NormalizedEnv(gym.ObservationWrapper):
    ...
    def observation(self, observation):
        ...
        self.state_std = self.state_std * self.alpha + \
            np.quantile(observation,0.98) * (1 - self.alpha)
