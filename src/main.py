...
if __name__ == "__main__":
    ...
    args["Training"]["shared_optimizer"] = str2bool(
        args["Training"]["shared_optimizer"]
    )
    ...
    args["Model"]["S_init_std_multiplier"] = float(
        args["Model"]["S_init_std_multiplier"]
    )
    ...
    shared_model = torch.nn.Sequential(
        Level1(args, env.observation_space.shape[0], env.action_space, device="cpu"),
    ...
    optimizer = SharedAdam(
        [
            {
                "params": shared_model[0].parameters(),
                "lr": args["Training"]["lr"],
            },  # 0.001},
            {"params": shared_model[1].parameters(), "lr": args["Training"]["lr"]},
        ],  # *0.05
        lr=args["Training"]["lr"],
    ...
    p = mp.Process(
        target=test, args=(args, shared_model, env_conf, counter, parallel_running_num)
    )
    p.start()
    processes.append(p)
    time.sleep(0.1)
