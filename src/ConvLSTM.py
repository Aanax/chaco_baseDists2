...
class ConvLSTMwithAbaseCell(nn.Module):
    def __init__(self, input_dim, hidden_dim, kernel_size, bias, num_actions):
        ...
        self.conv = nn.Conv2d(
            in_channels=self.input_dim + self.hidden_dim + self.num_actions,
            out_channels=4 * self.hidden_dim,
            kernel_size=self.kernel_size,
            padding=self.padding,
            bias=self.bias,
    ...
    def forward(self, input_tensor, cur_state, input_action):
        ...
        action_maps = (
            input_action.squeeze().unsqueeze(1).unsqueeze(2).expand((1, 6, 20, 20))
        )

        # spaial features
        # 1,32,20,20 + 1,32,20,20 + 1,6,20,20
        combined = torch.cat(
            [input_tensor, h_cur, action_maps], dim=1
        )  # concatenate along channel axis
        combined_conv = self.conv(combined)
        cc_i, cc_f, cc_o, cc_g = torch.split(combined_conv, self.hidden_dim, dim=1)
