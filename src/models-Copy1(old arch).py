...
class Level1(nn.Module):
    def __init__(self, args, shap, n_actions, device):
        ...
        self.ConvLSTM_logvar = ConvLSTMCell(
            input_dim=32,
            hidden_dim=32,
            kernel_size=(11, 11),
            #                                  num_layers=4,
    ...
    def forward(self, x):
        ...
        mu, cx11 = self.ConvLSTM_mu(
            s, (hx1[0], cx1[0])
        )  # (states[0][0][0],states[0][1][0]))
        logvar, cx12 = self.ConvLSTM_logvar(
            s, (hx1[1], cx1[1])
        )  # (states[0][0][1],states[0][1][1]))
        ...
        return (
            ...
            v,
            a,
            torch.stack([hx11, hx12]),
            torch.stack([cx11, cx12]),
            s,
            S,
        )
...
class Level2(nn.Module):
    ...
    def forward(self, x):
        ...
        #         x = torch.unsqueeze(x,1)
        mu, cx21 = self.ConvLSTM_mu(
            s, (hx2[0], cx2[0])
        )  # (states[0][0][0],states[0][1][0]))
        logvar, cx22 = self.ConvLSTM_logvar(
            s, (hx2[1], cx2[1])
        )  # (states[0][0][1],states[0][1][1]))
        #         std, cx_std = self.ConvLSTM_std(x)
        ...
        return (
            ...
            v,
            a,
            torch.stack([hx21, hx22]),
            torch.stack([cx21, cx22]),
            s,
            S,
        )
