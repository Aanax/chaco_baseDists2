...
def init_lstm_states(player):
    ...
    if player.gpu_id >= 0:
        with torch.cuda.device(player.gpu_id):
            ...
            player.hx2 = Variable(
                torch.zeros((1, 64, 5, 5)), requires_grad=requires_grad
            ).cuda()
            player.cx2 = Variable(
                torch.zeros((1, 64, 5, 5)), requires_grad=requires_grad
            ).cuda()
...
class Agent(object):
    def __init__(self, model1, model2, env, args, state, gpu_id):
        ...
        self.S2_prev = torch.zeros((1, args["Model"]["S2_dim"])).to(
            "cuda:" + str(gpu_id)
        )
    ...
    def action_train(self):
        ...
        with torch.autograd.set_detect_anomaly(True):
            kld1, x_restored1, v1, a1, self.hx1, self.cx1, s1, S1 = self.model1(
                (
                    Variable(self.state.unsqueeze(0)),
                    self.hx1,
                    self.cx1,
                    self.prev_action_logits.detach(),
                )
            )
        ...
        (
            prob_throughbase,
            log_prob_throughbase,
            action_throughbase,
            entropy_throughbase,
        ) = self.get_probs(a_throughbase)
    ...
    def action_test(self, ZERO_ABASE=False):
        with torch.no_grad():
            ...
            (
                ...
                log_prob2,
                kl_actor2,
            ) = self.model2((S1, self.hx2, self.cx2))
