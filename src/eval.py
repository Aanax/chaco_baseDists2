...
args.read(
    "/s/ls4/users/aamore/BaseDists_ver_before_sVAE_hevyside3/configs/a3c_with_vae.ini"
)  # sys.argv[1])#"./configs/star_config.ini"
...
args["Training"]["adaptive_gamma_and_eps"] = str2bool(
    args["Training"]["adaptive_gamma_and_eps"]
)
...
args["Training"]["w_restoration_future"] = float(
    args["Training"]["w_restoration_future"]
)
...
model_params_dict = args["Model"]
_model1 = Level1(
    args, env.observation_space.shape[0], env.action_space, device="cuda:" + str(gpu_id)
)
_model2 = Level2(
    args, env.observation_space.shape[0], env.action_space, device="cuda:" + str(gpu_id)
)
...
d1 = saved_state.__class__(
    {key[2:]: value for (key, value) in saved_state.items() if key[0] == "0"}
)
d2 = saved_state.__class__(
    {key[2:]: value for (key, value) in saved_state.items() if key[0] == "1"}
)
    ...
    while True:
        ...
        if render:
            if i_episode % render_freq == 0:
                ...
                img_orig = (
                    np.rollaxis(x_orig, 0, 3) * player.env.unbiased_std
                    + player.env.unbiased_mean
                ).astype("uint8")
                ...
                x_rest_ = (
                    x_rest * player.env.unbiased_std + player.env.unbiased_mean
                )  # 127.5
                img_rest = (np.rollaxis(x_rest_, 0, 3)).astype("uint8")
                _, thresholded_o = cv2.threshold(
                    img_rest, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU
                )
        ...
        if watch_z:
            ...
            if len(Vs) >= 2:
                delta_t2 = (1 - gamma1) * Vs[-2] + gamma2 * Vs2[-1] - Vs2[-2]
                deltas2.append(delta_t2)
                ##+ delta_t2 ?
                delta_t1 = rewards[-2] + gamma1 * Vs[-1] - Vs[-2]
        ...
        if player.done and not player.info:
            ...
            saveanimation(
                ...
                address=LOGSFOLDER
                + model_path.split("/")[-1].split(".")[0]
                + "normalized_vae_dis.mp4",
            )
            saveanimation(
                frames_restored,
                address=LOGSFOLDER
                + model_path.split("/")[-1].split(".")[0]
                + "restored_vae_dis.mp4",
            )
            saveanimation(
                frames_restored_thr,
                address=LOGSFOLDER
                + model_path.split("/")[-1].split(".")[0]
                + "restored_vae_THRESH_dis.mp4",
            )
            ...
            saveanimation(
                ...
                address=LOGSFOLDER
                + model_path.split("/")[-1].split(".")[0]
                + "from_lstm_vae_dis.mp4",
            )
            saveanimation(
                frames_from_lstm_thr,
                address=LOGSFOLDER
                + model_path.split("/")[-1].split(".")[0]
                + "from_lstm_THRESH_vae_dis.mp4",
            )
            saveanimation(
                frames_from_lstm_ranged,
                address=LOGSFOLDER
                + model_path.split("/")[-1].split(".")[0]
                + "from_lstm_RANGED.mp4",
            )
            ...
            if watch_z:
                ...
                with open(
                    LOGSFOLDER
                    + model_path.split("/")[-1].split(".")[0]
                    + "Frames_normalized_orig.npy",
                    "wb",
                ) as f:
                    np.save(f, np.array(frames_normalized_orig))
                with open(
                    LOGSFOLDER + model_path.split("/")[-1].split(".")[0] + "hx1.npy",
                    "wb",
                ) as f:
                    np.save(f, np.array(player.hx1.detach().cpu().numpy()))
                with open(
                    LOGSFOLDER + model_path.split("/")[-1].split(".")[0] + "hxs1.npy",
                    "wb",
                ) as f:
                ...
                with open(
                    LOGSFOLDER + model_path.split("/")[-1].split(".")[0] + "hx2.npy",
                    "wb",
                ) as f:
                ...
                with open(
                    LOGSFOLDER + model_path.split("/")[-1].split(".")[0] + "Ss.npy",
                    "wb",
                ) as f:
                ...
                with open(
                    ...
                    np.save(f, np.array(ss))
                with open(
                    LOGSFOLDER + model_path.split("/")[-1].split(".")[0] + "Vs.npy",
                    "wb",
                ) as f:
                    np.save(f, np.array(Vs))

                with open(
                    LOGSFOLDER + model_path.split("/")[-1].split(".")[0] + "Ss2.npy",
                    "wb",
                ) as f:
                ...
                with open(
                    ...
                    np.save(f, np.array(ss2))
                with open(
                    LOGSFOLDER + model_path.split("/")[-1].split(".")[0] + "Vs2.npy",
                    "wb",
                ) as f:
                    np.save(f, np.array(Vs2))
                with open(
                    LOGSFOLDER + model_path.split("/")[-1].split(".")[0] + "aas2.npy",
                    "wb",
                ) as f:
                    np.save(f, np.array(aas2))

                with open(
                    LOGSFOLDER + model_path.split("/")[-1].split(".")[0] + "aas1.npy",
                    "wb",
                ) as f:
                ...
                with open(
                    LOGSFOLDER + model_path.split("/")[-1].split(".")[0] + "aas.npy",
                    "wb",
                ) as f:
