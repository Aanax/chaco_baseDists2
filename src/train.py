...
#     player.VD_runningmean = VD_runningmean
    ...
    for i in reversed(range(len(player.rewards1))):
        ...
        ce_loss1 += -0.0*(torch.sum(player.probs_base[i].detach()*torch.log(player.probs1[i])))*(abs(player.values2[i].detach()))*abs(gae1)

        policy_loss_base = policy_loss_base - \
            player.log_probs1_throughbase[i] * gae2

        
        ce_loss_base += -0.0*(torch.sum(player.probs_play[i].detach()*torch.log(player.probs_throughbase[i])))*(abs(player.values1[i].detach()))*abs(gae2)
