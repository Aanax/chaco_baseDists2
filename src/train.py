...
def train(
    ...
    counter,
    num,
    main_start_time="unknown_start_time",
):
    ...
    while True:
        ...
        if not player.done:
            state = player.state
            kld1, x_restored1, v1, a1, hx1, cx1, s1, S1 = player.model1(
                (
                    Variable(state.unsqueeze(0)),
                    player.hx1,
                    player.cx1,
                    player.prev_action_logits.detach(),
                )
            )
        ...
        with torch.autograd.set_detect_anomaly(True):
            ...
            loss1 = (
                args["Training"]["w_policy"] * policy_loss1
                + args["Training"]["w_value"] * value_loss1
                + args["Training"]["w_policy"] * ce_loss1
            )
            loss2 = (
                args["Training"]["w_policy"] * policy_loss2
                + args["Training"]["w_value"] * value_loss2
                + args["Training"]["w_policy"] * policy_loss_base
                + args["Training"]["w_policy"] * ce_loss_base
            )
            ...
            writer = csv.writer(f)
            writer.writerow(
                [mean_V1, mean_V2, mean_re1, counter.value, local_counter]
                + additional_logs
            )
            f.close()
...
def get_pixel_change(pic1, pic2, STEP=20):
    ...
    for n1, i in enumerate(range(0, max_side, STEP)):
        for n2, j in enumerate(range(0, max_side, STEP)):
            res[n1, n2] = (
                (
                    pic1[:, i : i + STEP, j : j + STEP]
                    - pic2[:, i : i + STEP, j : j + STEP]
                )
                .mean()
                .abs()
            )
...
def train_A3C_united(
    ...
    for i in reversed(range(len(player.rewards1))):
        ...
        S_last1, part_S_loss1, delta_S1 = MPDI_loss_calc1(
            player, V_last1, S_last1, tau, gamma1, None, i
        )
        S_last2, part_S_loss2 = MPDI_loss_calc2(
        ...
        delta_t1 = (
            player.rewards1[i]
            + r1_bonus
            + gamma1 * player.values1[i + 1].data
            - player.values1[i].data
        )
        ...
        delta_t2 = (
            (1 - gamma1) * (player.values1[i].detach() + D1)
            + gamma2 * player.values2[i + 1].data
        ...
        restoration_loss1_part = (player.restoreds1[i] - player.restore_labels1[i]).pow(
            2
        ).sum() / 20
        restoration_loss2_part = (player.restoreds2[i] - player.restore_labels2[i]).pow(
            2
        ).sum() / 20
